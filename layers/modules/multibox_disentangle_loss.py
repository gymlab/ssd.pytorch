...
from ..box_utils import match, log_sum_exp, match_mask
from data import VOC_OBJ_MIN, VOC_OBJ_MAX, COCO_OBJ_MIN, COCO_OBJ_MAX


class MultiBoxDisentangledLoss(nn.Module):
    ...
    def __init__(self, num_classes, overlap_thresh, prior_for_matching,
                 ...
                 use_gpu=True):
        super(MultiBoxDisentangledLoss, self).__init__()
    ...
    def forward(self, predictions, targets):
        ...
        """
        loc_data, conf_data, priors, priorsd, disentangled_ftrs, mask = predictions
        num = loc_data.size(0)
        priors = priors[:loc_data.size(1), :]
        num_priors = (priors.size(0))
        num_priors_disentangled = (priorsd.size(0))

        # match priors (default boxes) and ground truth boxes
        loc_t = torch.Tensor(num, num_priors, 4)
        conf_t = torch.LongTensor(num, num_priors)
        mask_t = torch.LongTensor(num, num_priors_disentangled)
        for idx in range(num):
            truths = targets[idx][:, :-1].data
            labels = targets[idx][:, -1].data
            defaults = priors.data
            match(self.threshold, truths, defaults, self.variance, labels, loc_t, conf_t, idx)
            defaults = priorsd.data
            match_mask(self.threshold, truths, defaults, labels, mask_t, idx)
        if self.use_gpu:
            loc_t = loc_t.cuda()
            conf_t = conf_t.cuda()
            mask_t = mask_t.cuda()
        ...
        pos_m = mask_t > 0
        # Compute max conf across batch for hard negative mining
        loss_m = - torch.log(1. - mask.view(-1))

        # Hard Negative Mining
        loss_m[pos.view(-1, 1)] = 0  # filter out pos boxes for now
        loss_c = loss_c.view(num, -1)
        _, loss_idx = loss_c.sort(1, descending=True)
        _, idx_rank = loss_idx.sort(1)
        num_pos = pos.long().sum(1, keepdim=True)
        ...
        loss_l /= N
        loss_c /= N
        return loss_l, loss_c, loss_m, loss_adv, loss_d
